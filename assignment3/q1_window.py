...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(
            embeddings, [-1, n_window_features * embed_size])
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        weights_shape = ((2 * window_size + 1) * embed_size) * n_word_features

        W = tf.get_variable(name='input_weights',
                            shape=[weights_shape, hidden_size],
                            initializer=tf.contrib.layers.xavier_initializer())
        U = tf.get_variable(name='output_weights',
                            shape=[hidden_size, n_classes],
                            initializer=tf.contrib.layers.xavier_initializer())
        b1 = tf.get_variable(name='input_bias',
                             shape=[hidden_size],
                             initializer=tf.zeros_initializer())
        b2 = tf.get_variable(name='outpue_bias',
                             shape=[n_classes],
                             initializer=tf.zeros_initializer())
